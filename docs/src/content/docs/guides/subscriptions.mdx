...
|----------|---------|-------------------|
| Node.js | `@effect-gql/node` | Via `ws` package |
| Bun | `@effect-gql/bun` | Built-in |
| Express | `@effect-gql/express` | Via `ws` package |

All platforms use the same `graphql-ws` protocol, ensuring client compatibility.

## Next Steps

- [Server Integration](/effect-gql/guides/server-integration) - HTTP server setup
