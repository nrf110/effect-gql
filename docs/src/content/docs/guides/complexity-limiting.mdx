...
Sometimes you want to **report** complexity metrics to clients without blocking requests. The Analyzer extension adds complexity information to the response's `extensions` field, similar to [async-graphql's Analyzer](https://async-graphql.github.io/async-graphql/en/extensions_available.html).
